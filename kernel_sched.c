...
rlnode SCHED[RANK]; /* The scheduler queue */
...
static TCB* sched_queue_select()
{
	...
	{
		switch(is_rlist_empty(&SCHED[priority]))
		{
			case 0:
				selected_node = rlist_pop_front(&SCHED[priority]);
				break;
			case 1:
				not_found++;
				continue;
...
/* Get next thread */
  ...
  /* If there is nothing ready in the scheduler queue ? */
  if(next==NULL) {
    if(current_thread_ready)
      next = current;
    else
...
/* Priority booster for the lower prioritized threads */
