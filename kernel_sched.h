...
/**
  ...
  An object of this type is associated to a single thread. It is used for going back and forth between threads.
*/
...
typedef struct process_thread_control_block{
  ...
  CondVar cv;     /* CV to open the PTCB */ 

  int argl;
  void* args;

  int exitval;    /*The exit value of the thread*/
  int exited;     /*If the thread is finished it is 1 otherwise it is 0*/
  int joinable;   /*If the thread is joinable it is 1 otherwise it is 0*/
  int wt_counter;/*How many threads are waiting for a thread in the PTCB*/
...
/**
  @brief Priority Booster

  This function is called before the new thread is selected and if enough time has passed
  */
